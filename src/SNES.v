...
module SNES(
    ...
    output [23:0] RGB_OUT /*verilator public*/,
    output HDE,
    output VDE,
    output HSYNC,
    output VSYNC,
...
// SMP
SMP smp(
   .CLK(DSPCLK), .RST_N(RST_N), .CE(SMP_CE), .EN_R(SMP_EN_R), .EN_F(SMP_EN_F), .SYSCLKF_CE(INT_SYSCLKF_CE),
   .A(SMP_A), .DI(SMP_DI), .DO(SMP_DO), .WE(SMP_WE),
...
// DSP 
DSP dsp(
    .CLK(DSPCLK), .RST_N(RST_N), .ENABLE(ENABLE), .PAL(PAL), .FREQ(1'b0),
    .SMP_EN_R(SMP_EN_R), .SMP_EN_F(SMP_EN_F), .SMP_A(SMP_A), .SMP_DO(SMP_DO), .SMP_DI(SMP_DI),
    .SMP_WE(SMP_WE), .SMP_CE(SMP_CE),
