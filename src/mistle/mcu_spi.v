...
module mcu_spi (
  ...
  always @(negedge spi_io_clk or posedge spi_io_ss) begin
    if (spi_io_ss) begin
      ...
      // be increased.
      if (spi_cnt[2:0] == 3'd3) spi_data_in_ready <= 1'b0;
    end
  ...
  always @(posedge clk) begin
    ...
    if (spi_data_in_readyD == 2'b01) begin
      ...
      if (spi_in_cnt == 4'd0) spi_target <= spi_data_in;
      else begin
        spi_in_strobe <= 1'b1;
        spi_in_data   <= spi_data_in;
      end

      if (spi_in_cnt != 4'd15) spi_in_cnt <= spi_in_cnt + 4'd1;
    end else spi_in_strobe <= 1'b0;
  end
